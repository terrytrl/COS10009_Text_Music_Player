...
class Album
    attr_accessor :index, :title, :artist, :genre, :tracks
    def initialize( index, title, artist, genre, tracks)
      @index = index
        @title = title
        @artist = artist
        @genre = genre
...
def print_album_names(albums)
  if (!albums)
      ...
      count = albums.length
      while (i < count)
          puts ("Album #{index + 1}:" + albums[i].title)
          i += 1
      end
...
def update_album_title()
  albums[album_select - 1].title = read_string("Enter updated title: ")
                puts ("Updated Title is: " + albums[album_select - 1].title)
                modify_menu = false
end
